...
```{r}
mean(MyData$Egg_Size_g, na.rm=TRUE)
```
...
```{r, eval=FALSE}
MyData %>% filter(!is.na(MyData$Egg_Size_g)) #all the rows that are not NAs
MyData %>% filter(is.na(MyData$Egg_Size_g))  #all the rows that are NAs
```

Let's clean up our data to omit rows with missing values, and use this dataset going forward.

```{r}
MyData <- MyData %>% filter(!is.na(MyData$Egg_Size_g)) 
```

Let's try some simple functions using the pipe operator.

```{r, eval=FALSE}
MyData %>% filter(N_Eggs>=5) #filtering out all the rows which its number of eggs is greater than or equal to 5. 
MyData %>% filter(Site=="SiteA") #filtering out all the rows that are collected at SiteA
MyData %>% filter(Site=="SiteA", Family=="Cardinalidae") #filtering for both SiteA and Family Cardinalidae
MyData %>% filter(N_Eggs>=5,Family=="Cardinalidae") #filtering for both egg number greater than or equal to 5 and matching the family Cardinalidae using a comma. 
MyData %>% filter(N_Eggs>=5|N_Eggs<3) #filtering for individuals with number of eggs above or equal to 5 OR below 3, using the | symbol.
```

This is an example of arranging the egg size in increasing order and removing missing values in the egg size column. 

```{r, eval=FALSE}
MyData %>% arrange(Egg_Size_g,na.rm=TRUE)
```
...
```{r}
MyData %>% 
  filter (!is.na(Egg_Size_g)) %>% 
  group_by(Family) %>% 
  summarise(average_egg_size_g=mean(Egg_Size_g),
            sd=sd(Egg_Size_g))
```
...
head(Egg_size_data) # checking the data frame.
```
...
```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g)
```
...
```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g, xlab = "Body Size (g)", ylab = "Egg Size (g)", main = "Birds Lay Eggs")
```
...
```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g, xlab = "Body Size (g)", ylab = "Egg Size (g)", main = "Birds Lay Eggs", pch = 19)
```
...
```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g, xlab = "Body Size (g)", ylab = "Egg Size (g)", main = "Birds Lay Eggs", pch = 19, col = "red")
```

Method 2: Using the colour's hex value.

```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g, xlab = "Body Size (g)", ylab = "Egg Size (g)", main = "Birds Lay Eggs", pch = 19, col = "#FF0000")
```
...
```{r}
plot(MyData$Body_Size_g, MyData$Egg_Size_g, xlab = "Body Size (g)", ylab = "Egg Size (g)", main = "Birds Lay Eggs", pch = 19, col = "red", cex = 3)
```
...
```{r}
ggplot(aes(x = Body_Size_g, y = MyData$Egg_Size_g), data = MyData) +
  geom_point()
```
...
ggplot(aes(x = Body_Size_g, y = MyData$Egg_Size_g), data = MyData) +
  geom_point() +
  labs(x = "Body Size (g)", y = "Egg Size (g)", title = "Birds Lay Eggs")
```
...
```{r}
ggplot(aes(x = Body_Size_g, y = MyData$Egg_Size_g), data = MyData) +
  geom_point(colour = rgb(0.25, 0.15, 0.45)) +
  labs(x = "Body Size (g)", y = "Egg Size (g)", title = "Birds Lay Eggs")
```
...
```{r}
MyData3 <- filter(MyData, Family == "Emberizidae")
```
...
```{r}
ggplot(data = MyData3, mapping = aes(x = Egg_Size_g)) + geom_histogram(bins = 100)
```
...
```{r}
ggplot(data = MyData3, mapping = aes(x = Egg_Size_g)) + geom_histogram(bins = 100, colour = "white", fill = "blue")
...
```{r}
ggplot(data = MyData, mapping = aes(x = Egg_Size_g, fill = Family)) + geom_histogram(bins = 100)
...
```{r}
ggplot(data = MyData, mapping = aes(x = Egg_Size_g, fill = Family)) + geom_histogram(bins = 100) + labs(x = "Size of egg (g)", title = "Distribution of bird egg size")
...
```{r}
ggplot(data = MyData, mapping = aes(x = Egg_Size_g, fill = Family)) + geom_histogram(bins = 100) + labs(x = "Size of egg (g)", title = "Distribution of bird egg size") + xlim(c(0,20)) + ylim(c(0, 60)) + theme(legend.position = "right")
...
```{r}
ggplot(data = MyData, mapping = aes(x = Egg_Size_g)) + geom_histogram(bins = 100) + labs(x = "Size of egg (g)", title = "Distribution of bird egg size") + xlim(c(0,20)) + ylim(c(0, 60)) + facet_grid(vars(Family))
...
```{r}
ggplot(data = MyData, aes(x = Family, y = Egg_Size_g)) + geom_boxplot()
```
...
```{r}
ggplot(data = MyData, aes(x = Family, y = Egg_Size_g)) + geom_boxplot() + coord_flip()

```
You can change labels

```{r}
ggplot(data = MyData, aes(x = Family, y = Egg_Size_g)) + geom_boxplot() + labs(y = "Size of egg (g)", x = "Family", title = "Size of egg for bird families") 
...
```{r}
ggplot(data = MyData, aes(x = Family, y = Egg_Size_g, fill = Site)) + geom_boxplot() + labs(y = "Size of egg (g)", x = "Family", title = "Size of egg for bird families") + theme(legend.position = "right")
...
```{r}
ggplot(data = MyData, mapping = aes(x = Egg_Size_g)) + geom_histogram(bins = 100) 
```
...
```{r}
ggplot(data = MyData, mapping = aes(x = log(Egg_Size_g))) + geom_histogram(bins = 100)
```
...
```{r}
ggplot(aes(x = Body_Size_g , y = Egg_Size_g), data = MyData) +
  geom_point(alpha = 0.5, size = 5, color = "orange") +
  labs(x = "Body Size (g)", y = "Egg Size (g)")
...
```{r}
hist(MyData$Egg_Size_g)
hist(MyData$Body_Size_g)
```

```{r}
MyData$Egg_Size_g <- log(MyData$Egg_Size_g)
MyData$Body_Size_g <- log(MyData$Body_Size_g)
```

```{r}
hist(MyData$Egg_Size_g)
hist(MyData$Body_Size_g) #that's better!
```

```{r}
LinMod <- lm(Body_Size_g ~ Egg_Size_g, data=MyData)
LinMod
```
...
```{r}
unique(MyData$Family)
```


```{r}
AOVMod <- lm(Body_Size_g~Family, data=MyData)
summary(AOVMod)
anova(AOVMod)
```
...
```{r}
MultRegMod <- lm(Body_Size_g ~ Egg_Size_g + Duration, data=MyData)
summary(MultRegMod)
anova(MultRegMod)
```
...
```{r}
MultAOVMod <- lm(Body_Size_g ~ Family + Genus, data=MyData)
summary(MultAOVMod)
anova(MultAOVMod)
```
...
```{r}
ANCMod <- lm(Body_Size_g ~ Egg_Size_g + Family, data=MyData)
summary(ANCMod)
anova(ANCMod)
```
...
```{r}
IntMod <- lm(Body_Size_g ~ Egg_Size_g + Family + Egg_Size_g:Family, data=MyData)
summary(IntMod)
anova(IntMod)
```
...
```{r}
ggplot()+
  geom_histogram(aes(x=residuals(AOVMod)))
ggplot()+
  geom_histogram(aes(x=residuals(LinMod)))
```
...
```{r}
ggplot() + 
  stat_qq(aes(sample=residuals(AOVMod))) + 
  stat_qq_line(aes(sample=residuals(AOVMod)))

ggplot() + 
  stat_qq(aes(sample=residuals(LinMod))) + 
  stat_qq_line(aes(sample=residuals(LinMod)))
```
...
```{r}
MyData$Residuals<-residuals(AOVMod)
ggplot(aes(x=Family, y=Residuals), 
       data=MyData) +
  geom_boxplot()

MyData$Residuals<-residuals(AOVMod)
ggplot(aes(x=Egg_Size_g, y=Residuals), 
       data=MyData) +
  geom_point()
```
...
```{r}
GLMod<-glm(Body_Size_g~Egg_Size_g, family=gaussian, data=MyData)
```
...
```{r}
PoisMod<-glm(N_Eggs ~ Egg_Size_g*Family, family=poisson, data=MyData)
PoisMod
```
...
```{r}
BinMod<-glm(Overwinter_Survival ~ Body_Size_g*Family, family=binomial, data=MyData)
BinMod
```
...
```{r}
MixedMod <- lmer(Body_Size_g~ Egg_Size_g + (1|Family) + (1|Genus), data=MyData)
summary(MixedMod)
```
...
```{r}
MixedMod2 <- lmer(Body_Size_g~ Egg_Size_g + (1|Family), data=MyData)
lrtest(MixedMod2, MixedMod)
```
...
```{r}
Song<-MyData %>% dplyr::select(1:5, 10:14)
head(Song)
```
...
```{r, error=T}
cor(Song[6:10]) #if we had missing data we would use the use="pairwise.complete.obs"
```
...
```{r}
SongPCA<-princomp(Song[6:10], cor=T)
```
...
```{r}
PCloadings<-data.frame(Component=c(1:5),
                        Eigenvalue=SongPCA$sdev^2)
ggplot(aes(x=Component,y=Eigenvalue),data=PCloadings) +
...
```{r}
testDat<-Song %>%
  mutate(PCcalc=scale(Duration)*Load[1] + 
           scale(RepeatLen)*Load[2] +
           scale(PitchRange)*Load[3] +
           scale(AmplitudeRange)*Load[4] +
          scale(Ramping)*Load[5])

testDat$Comp.1<-SongPCA$scores[,1]
...
```{r}
Song$PC1<-SongPCA$scores[,1]
Song$PC2<-SongPCA$scores[,2]
head(Song)
```
...
```{r}
Mod<-lm(PC1 ~ Family + Genus + Species + Site, data=Song)
anova(Mod)
```
...
LDAMod <- lda(Family ~ Duration + RepeatLen + PitchRange + AmplitudeRange + Ramping, data = MyData)
...
```{r}
CatDat <- data.frame(Observed = as.factor(MyData$Family),
                     Predicted = PredOut$class)
...
```{r}
sum(diag(table(CatDat)))/sum(table(CatDat))
```
...
ggplot(aes(x=PC1,y=PC2,colour=Family),data=Song) +
  geom_point() + theme(legend.position = "bottom") +
  xlab("PC1 (54.56%)") +
  ylab("PC2 (25.26%)") +  
  geom_point(alpha = 0.1, size = 12) + mycolors +stat_ellipse()
...
## LDA Plot
```{r}
LDA_df <- data.frame(ld1 = PredOut$x[,1], ld2 = PredOut$x[,2]) #Selecting the first two loadings
LDA_df <- data.frame(LDA_df, Song) #Using Song here which is a smalled data fram we had created for the PCA

LDA_Plot <- ggplot(data = LDA_df, aes(x=ld1,y=ld2,colour=Family)) + geom_point() +
  xlab("LD1") +
  ylab("LD2") +
  theme(legend.position = "bottom") +  mycolors
LDA_Plot
