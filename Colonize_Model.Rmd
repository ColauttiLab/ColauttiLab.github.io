...
SimDat<-Root(p1=0.421,p2=0.612,a=0.142,N=100000)
...
```{r}
Pdat<-as.data.frame(acf(test$batch))
names(Pdat)<-c("p1","p2","a")
Pdat$Iter<-seq_along(Pdat$a)
qplot(x=Iter,y=p1,data=Pdat,geom="line")+theme_bw()
qplot(x=Iter,y=p2,data=Pdat,geom="line")+theme_bw()
qplot(x=Iter,y=a,data=Pdat,geom="line")+theme_bw()
```
